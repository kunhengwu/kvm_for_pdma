...
void init_queue(struct host_pdma *pdma){
...
int init_pdma(struct host_pdma *pdma, char *device_path){
	int ret;
	int fd;
...
int get_pdma_info(struct host_pdma *pdma){
	int ret;	
...
int main(int argc, char *argv[]){
	...
	*/
	for(i = 0; i < host_pdma_control.pdma_dev_number; ++i){
		init_pdma(&host_pdma[i], device_path[i]);
		if(get_pdma_info(&host_pdma[i])){
	...
	for(i = 0; i < argc - 1; i++){
		add_event(epollfd, host_pdma.socket_fds[i], EPOLLIN);
	}
	// for(i = 0; i < argc - 1; i++){
