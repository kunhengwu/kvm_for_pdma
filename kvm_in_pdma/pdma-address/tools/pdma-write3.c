...
int main(int argc, char *argv[])
{
    ...
    sys_tm_get_us(&tm_beg);
     int sin_len = 128/2;
    for(i =0;i<info.wt_block_sz/(sin_len*2*2);i++)
	for(j=0;j<sin_len;j++){

           ((short *)buf)[(j<<1)+(i*sin_len<<1)] = 2*1024*sin(2*3.1415926*j/sin_len);//+1024*sin(2*3.1415926*j/(sin_len/16)); 
	  
	   ((short *)buf)[(j<<1)+1+(i*sin_len<<1)] = 2*1024*cos(2*3.1415926*j/sin_len);//+1024*cos(2*3.1415926*j/(sin_len/16)); 
            
           
	   fprintf(f1,"%d %d\n", ((short *)buf)[(j<<1)+(i*sin_len<<1)], ((short *)buf)[(j<<1)+(i*sin_len<<1)+1]);	
        }
    count=0;
    while (1) 
    {
